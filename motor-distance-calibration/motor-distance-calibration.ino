...
int direction = -10; //find the negative limit first to set home point
int count = 0;
  ...
  if((millis() - my_switch.last_switch_debounce_time) > DEBOUNCE_DELAY)
  {
    if(reading != my_switch.switch_state)
    {
      ...
      if(my_switch.switch_state == HIGH)
      {
        ...
        if(home_found && my_switch.limit < 0)
        {
          my_switch.limit = current_position;
        }
        else if(my_switch.limit < 0)
        {
          home_found = true;
          my_switch.limit = 0; //home point found 
        }
...
void loop() {
  ...
  if(is_lower_hit && upper_switch.limit < 0)
  {
    direction = direction * -1;
    //myStepper.moveTo(20000);
  }
  else if(is_lower_hit)
  {
    myStepper.moveTo(upper_switch.limit);
  }
  bool is_upper_hit = read_pin(upper_switch);
  if(is_upper_hit && lower_switch.limit < 0)
  {
    direction = direction * -1;
    //myStepper.moveTo(20000);
  }
  else if(is_upper_hit)
  {
    myStepper.moveTo(lower_switch.limit);
  }

  if(!is_lower_hit && !is_upper_hit)
  {
    myStepper.move(direction);
    myStepper.setSpeed(200);
    myStepper.run();
  }
  count = count + 1;
